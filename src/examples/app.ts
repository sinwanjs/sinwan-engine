...
const requireAuth: RouteHandler = (ctx) => {
  ctx.emitAsync("app:custom", { info: "Auth check in middleware" });
...
bus.off("app:custom", (_ctx, payload) => {
  console.log("[custom] hdhdhdhdhdhdhdhd", payload);
});
...
console.log(
  "  curl -H 'Authorization: Bearer secret-token' http://localhost:3000/api/v1/admin/dashboard",
);
