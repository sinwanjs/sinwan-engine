...
import type { SjsElement, SjsNode } from "../../view/types.ts";
...
import { HtmlEscapedString } from "../../view/jsx/jsx-runtime.ts";
import {
  parseTextOpenMarker,
  isTextCloseMarker,
  COMP_ID_ATTR,
...
function hydrateAttributes(
  ...
  for (const [key, value] of Object.entries(props)) {
    if (key === "children" || key === "key" || key === "ref" || isEventProp(key)) continue;
...
function hydrateComponent(
  ...
  try {
    ...
    return {
      type: "component",
      children: [{ type: "text", node: textNode ?? document.createTextNode("") }],
      disposers: [],
      instance,
    };
  }
